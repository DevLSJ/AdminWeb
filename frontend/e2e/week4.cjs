...
(async () => {
  ...
  try {
    ...
    await page.getByRole('row', { name: new RegExp(title + ' 수정') }).click();
    await page.getByRole('button', { name: '선택 삭제 (1)', exact: true }).click();
    await page.getByRole('dialog').getByRole('button', { name: '삭제', exact: true }).click();
    await page.getByText(title + ' 수정', { exact: true }).waitFor({ state: 'detached' });
    assert.equal((await request('/api/notices?title=' + encodeURIComponent(title))).totalElements, 0);
    check('대시보드에서 상세 이동·관리자 게시글 선택 삭제');
