            ...
            {visibleMenuGroups.map((group) => {
              ...
              const button = (
                <ListItemButton
                  ...
                  sx={{ minHeight: 48, justifyContent: showText ? 'initial' : 'center', borderRadius: 2.5, px: showText ? 1.5 : 0, color: '#fff', transition: 'background-color 220ms ease, color 220ms ease, transform 240ms cubic-bezier(.16,1,.3,1), box-shadow 220ms ease', '&.Mui-selected': { bgcolor: group.children ? 'rgba(4,43,122,.48)' : 'rgba(255,255,255,.18)', boxShadow: 'inset 3px 0 0 #b9d2ff, 0 8px 20px rgba(5,35,101,.16)', transform: 'translateX(2px)' }, '&.Mui-selected:hover': { bgcolor: group.children ? 'rgba(4,43,122,.58)' : 'rgba(255,255,255,.22)' }, '&:hover': { bgcolor: 'rgba(255,255,255,.12)', color: '#fff' } }}
                >
              ...
              return (
                <Box key={group.id} sx={{ mb: 0.6 }}>
                  ...
                  {group.children && showText && <Collapse in={groupOpen} timeout="auto" unmountOnExit><List disablePadding sx={{ position: 'relative', mt: .55, ml: 2.6, pl: 1.5, '&::before': { position: 'absolute', top: 7, bottom: 7, left: 0, width: 1.5, bgcolor: 'rgba(255,255,255,.28)', content: '""' } }}>{group.children.map((child) => {
                    ...
                    return <ListItem key={child.path} disablePadding><ListItemButton selected={selected} onClick={() => handleNavigate(child.path)} sx={{ minHeight: 42, mb: .35, px: 1.25, borderRadius: 2.5, color: selected ? '#123b86' : 'rgba(255,255,255,.78)', transition: 'background-color 220ms ease, color 220ms ease, transform 240ms cubic-bezier(.16,1,.3,1), box-shadow 220ms ease', '&.Mui-selected': { bgcolor: 'rgba(255,255,255,.96)', boxShadow: '0 8px 20px rgba(6,37,103,.16)', transform: 'translateX(3px)' }, '&.Mui-selected:hover': { bgcolor: '#fff' }, '&:hover': { bgcolor: selected ? '#fff' : 'rgba(255,255,255,.12)', color: selected ? '#123b86' : '#fff' } }}><ListItemIcon sx={{ minWidth: 32, color: 'inherit', '& .MuiSvgIcon-root': { fontSize: 18 } }}>{child.icon}</ListItemIcon><ListItemText primary={child.label} slotProps={{ primary: { sx: { fontSize: 14, lineHeight: 1.5, fontWeight: selected ? 750 : 550 } } }} /></ListItemButton></ListItem>
                  })}</List></Collapse>}
                </Box>
              )
            })}
