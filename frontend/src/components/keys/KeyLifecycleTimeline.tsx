...
export function KeyLifecycleTimeline({ histories, sticky = false, maxHeight = 'calc(100vh - 190px)' }: KeyLifecycleTimelineProps) {
  return (
    <Card className="section-card" sx={{ position: sticky ? { xl: 'sticky' } : undefined, top: sticky ? { xl: 92 } : undefined, maxHeight: { xl: maxHeight }, display: { xl: 'flex' }, flexDirection: 'column', overflow: 'hidden' }}>
      <Box className="section-card-header" sx={{ display: 'grid', flexShrink: 0, gridTemplateColumns: 'minmax(0,1fr) 68px', alignItems: 'center', gap: 1 }}>
      ...
      <Box className="timeline-scroll" sx={{ position: 'relative', maxHeight: { xs: maxHeight, xl: 'none' }, minHeight: { xs: 360, xl: 0 }, flex: { xl: 1 }, overflowY: 'auto', overscrollBehavior: 'contain', p: 1.5, '&::before': histories.length > 1 ? { content: '""', position: 'absolute', left: 31, top: 35, bottom: 35, width: 2, bgcolor: 'divider' } : undefined }}>
