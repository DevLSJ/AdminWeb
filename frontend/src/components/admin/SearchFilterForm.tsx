...
export function SearchFilterForm({ children, columns = 2, onSearch, onReset }: {
  children: ReactNode
  columns?: 2 | 3 | 4
  ...
  return (
    <FilterCard>
      <Box component="form" aria-label="검색 조건" onSubmit={onSearch}>
        <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0, 1fr)', sm: 'repeat(2, minmax(0, 1fr))', xl: `repeat(${columns}, minmax(0, 1fr))` }, gap: 1.5 }}>
          {children}
        </Box>
