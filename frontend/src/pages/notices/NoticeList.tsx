...
import { Alert, Box, Button, Card, CardContent, FormControl, FormHelperText, IconButton, InputAdornment, InputLabel, List, ListItem, ListItemText, MenuItem, Pagination, Select, Stack, Switch, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography } from '@mui/material'
...
import { FilterCard, PageHeader } from '../../components/admin/AdminPage'
      ...
      <Box className="notice-page" sx={{ width: '100%', maxWidth: '100%', overflowX: 'hidden' }}>
        ...
        {showEditor ? (
          <Card className="section-card" sx={{ maxWidth: 980, mx: 'auto', overflow: 'hidden', borderRadius: '9px !important' }}>
            <Box className="section-card-header" sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}><Typography variant="h6">{isCreate ? '새 게시글 작성' : '게시글 수정'}</Typography><StatusBadge label="첨부 암호화" tone="info" minWidth={0} /></Box>
            <Box component="form" onSubmit={(event) => void saveNotice(event)}>
              <CardContent sx={{ p: { xs: '18px !important', sm: '22px !important' } }}>
                <Stack spacing={2.25}>
                  <FormControl fullWidth size="small"><InputLabel id="board-category-label">카테고리</InputLabel><Select labelId="board-category-label" label="카테고리" value={form.category} onChange={(event) => setForm((current) => ({ ...current, category: event.target.value as BoardForm['category'] }))}><MenuItem value="GENERAL">일반 글</MenuItem><MenuItem value="NOTICE" disabled={!isAdmin}>공지사항 · 상단 고정</MenuItem></Select><FormHelperText>{isAdmin ? '공지사항은 게시판 최상단에 강조 표시됩니다.' : '공지사항 카테고리는 관리자만 선택할 수 있습니다.'}</FormHelperText></FormControl>
                  ...
                  <Stack direction={{ xs: 'column', sm: 'row' }} spacing={1} sx={{ p: 1.5, borderRadius: 2, bgcolor: (theme) => theme.palette.mode === 'light' ? '#edf4ff' : 'rgba(23,105,232,.12)', color: 'text.secondary' }}><LockRounded sx={{ flexShrink: 0, color: 'primary.main', fontSize: 18 }} /><Typography sx={{ fontSize: 11, lineHeight: 1.6 }}><Typography component="span" sx={{ mr: .8, color: 'primary.main', fontSize: 'inherit', fontWeight: 800 }}>보안 처리</Typography>브라우저는 파일 원문을 저장하지 않습니다. 실제 구현에서는 Spring 서비스가 12바이트 난수 IV로 암호화하고 다운로드 요청 때만 복호화합니다.</Typography></Stack>
                </Stack>
              </CardContent>
  ...
  return (
    <Box className="notice-page" sx={{ width: '100%', maxWidth: '100%', overflowX: 'hidden' }}>
      ...
      <Card className="section-card" sx={{ width: '100%', maxWidth: '100%', overflow: 'hidden', borderRadius: '2px !important' }}>
          ...
          <Table stickyHeader size="small" className="dense-data-table" sx={{ width: '100%', minWidth: 1000, tableLayout: 'fixed', '& .MuiTableCell-root': { px: 1.5, overflow: 'hidden', fontSize: 13.25, ...paginatedTableCellSx(params.size) }, '& .MuiTableCell-head': { py: 1, bgcolor: 'background.paper', fontSize: 13, fontWeight: 800 } }}>
            ...
            <TableBody>{notices.map((notice) => (
              <TableRow key={notice.noticeUid} hover tabIndex={0} className="interactive-row" sx={notice.category === 'NOTICE' ? { cursor: 'pointer', '& .MuiTableCell-root': { bgcolor: (theme) => theme.palette.mode === 'light' ? '#fff7ed' : 'rgba(249,115,22,.10)' }, '&:hover .MuiTableCell-root': { bgcolor: (theme) => theme.palette.mode === 'light' ? '#ffedd5' : 'rgba(249,115,22,.16)' }, '& .MuiTableCell-root:first-of-type': { borderLeft: '4px solid #f59e0b' } } : { cursor: 'pointer', '& .MuiTableCell-root': { bgcolor: 'background.paper' }, '&:hover .MuiTableCell-root': { bgcolor: 'action.hover' }, '& .MuiTableCell-root:first-of-type': { borderLeft: '4px solid transparent' } }} onClick={() => openDetail(notice)} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') openDetail(notice) }}>
                ...
                <TableCell><Typography sx={{ fontWeight: notice.category === 'NOTICE' ? 850 : 750, fontSize: 13.5 }}>{notice.title}</Typography><Typography sx={{ color: notice.category === 'NOTICE' ? '#c2670c' : 'text.secondary', fontSize: 11 }}>{notice.category === 'NOTICE' ? `상단 고정 · ${notice.noticeUid}` : notice.noticeUid}</Typography></TableCell>
                ...
                <TableCell><Typography sx={{ fontSize: 13.25, fontWeight: 650 }}>{notice.createdBy}</Typography></TableCell>
        ...
        <Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 1.5, px: 2, py: 1.25, borderTop: '1px solid', borderColor: 'divider' }}><Typography sx={{ color: 'text.secondary', fontSize: 12 }}>{totalElements === 0 ? '0건' : `${params.page * params.size + 1}–${Math.min((params.page + 1) * params.size, totalElements)} / ${totalElements.toLocaleString()}건`} · 공지 우선</Typography><Pagination count={Math.max(1, Math.ceil(totalElements / params.size))} page={params.page + 1} onChange={(_event, page) => updateParam('page', page - 1)} color="primary" size="small" siblingCount={1} boundaryCount={1} sx={{ '& .MuiPaginationItem-root': { minWidth: 32, height: 32, border: '1px solid', borderColor: 'divider', borderRadius: 1.25, fontWeight: 700 }, '& .Mui-selected': { borderColor: 'primary.main' } }} /></Box>
      </Card>
    </Box>
  )
}
