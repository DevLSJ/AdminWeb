  ...
  if (isDetail || isCreate) {
    ...
    return (
      <Box className="notice-page" sx={{ width: '100%', maxWidth: '100%', overflowX: 'hidden', ...(showEditor && { height: '100%', minHeight: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', '& > *': { flexShrink: 0 } }) }}>
        ...
        {showEditor ? (
          <Card className="section-card" sx={{ width: '100%', minHeight: 0, flex: '1 1 0 !important', display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: '9px !important', '& > .section-card-header': { flexShrink: 0 } }}>
            ...
            <Box component="form" onSubmit={(event) => void saveNotice(event)} sx={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
              <CardContent sx={{ p: { xs: '12px !important', sm: '18px !important' }, flex: 1, minHeight: 0, overflow: 'hidden' }}>
                <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1fr)', sm: 'minmax(0,1fr) minmax(240px,.55fr)' }, gridTemplateRows: { xs: 'minmax(0,1fr) auto', sm: 'minmax(0,1fr)' }, gap: 2, height: '100%', minHeight: 0 }}>
                <Stack spacing={1.5} sx={{ minWidth: 0, minHeight: 0, '& > *': { flexShrink: 0 } }}>
                  ...
                  <Box sx={{ flex: '1 1 0 !important', minHeight: 0, display: 'flex', flexDirection: 'column' }}><Stack direction="row" spacing={.5} sx={{ mb: .75 }}><Typography component="label" htmlFor="notice-content" sx={{ fontSize: 12.5, fontWeight: 750 }}>본문</Typography><Typography component="span" color="error.main" sx={{ fontSize: 12.5, fontWeight: 850 }}>*</Typography></Stack><TextField id="notice-content" required fullWidth multiline rows={1} sx={{ flex: 1, minHeight: 0, '& .MuiInputBase-root': { height: '100%', alignItems: 'stretch' }, '& textarea:not([aria-hidden])': { height: '100% !important', overflowY: 'auto !important', overscrollBehavior: 'contain' } }} placeholder="게시글 내용을 입력하세요" value={form.content} onChange={(event) => setForm((current) => ({ ...current, content: event.target.value }))} slotProps={{ htmlInput: { maxLength: 100000 } }} /><Typography sx={{ mt: .55, color: 'text.secondary', fontSize: 10.5, textAlign: 'right' }}>{form.content.length.toLocaleString()} / 100,000</Typography></Box>
                </Stack>
                <Stack spacing={1.5} sx={{ minWidth: 0, minHeight: 0, maxHeight: { xs: 150, sm: 'none' }, overflowY: 'auto', overscrollBehavior: 'contain' }}>
                  ...
                  <Box><Box component="label" htmlFor="notice-files" onDragEnter={(event) => { if (Array.from(event.dataTransfer.types).includes('Files')) { event.preventDefault(); fileDragDepth.current += 1; setDraggingFiles(true) } }} onDragLeave={() => { fileDragDepth.current = Math.max(0, fileDragDepth.current - 1); if (!fileDragDepth.current) setDraggingFiles(false) }} onDragOver={dragFilesOver} onDrop={dropFiles} data-dragging={draggingFiles} sx={{ display: 'grid', minHeight: 142, placeItems: 'center', p: 2.5, border: '1.5px dashed', borderColor: draggingFiles ? 'primary.main' : 'primary.light', borderRadius: 2, bgcolor: (theme) => draggingFiles ? (theme.palette.mode === 'light' ? 'rgba(23,105,232,.10)' : 'rgba(23,105,232,.20)') : (theme.palette.mode === 'light' ? 'rgba(23,105,232,.025)' : 'rgba(23,105,232,.08)'), textAlign: 'center', cursor: 'pointer', transition: 'border-color 180ms ease, background-color 180ms ease', '&:hover': { borderColor: 'primary.main', bgcolor: (theme) => theme.palette.mode === 'light' ? 'rgba(23,105,232,.05)' : 'rgba(23,105,232,.12)' } }}><Box><CloudUploadRounded sx={{ mb: .6, color: 'primary.main', fontSize: 27 }} /><Typography sx={{ fontSize: 12.5, fontWeight: 750 }}>첨부파일을 선택하거나 끌어놓으세요</Typography><Typography sx={{ mt: .45, color: 'text.secondary', fontSize: 10.5 }}>개별 10MB 이하 · 최대 10개 · 서버 저장 전 AES-256-GCM 암호화</Typography></Box><input id="notice-files" hidden multiple type="file" onChange={(event) => { addFiles(Array.from(event.target.files ?? [])); event.target.value = '' }} /></Box>
                    {(selectedNotice?.files.length ?? 0) + files.length === 0
                      ? <Typography sx={{ mt: 1, color: 'text.secondary', fontSize: 10.5 }}>첨부파일이 없습니다.</Typography>
                      : <Stack spacing={.75} sx={{ mt: 1 }}>
                          ...
                          {files.map((file, index) => (
                            <Box key={`${file.name}-${file.lastModified}`} sx={{ display: 'flex', alignItems: 'center', gap: 1, px: 1.25, py: .8, borderRadius: 1.5, bgcolor: 'action.hover' }}>
                              <AttachFileRounded sx={{ color: 'primary.main', fontSize: 17 }} />
              ...
              <Box sx={{ display: 'flex', justifyContent: 'flex-end', gap: 1, flexShrink: 0, px: { xs: 2, sm: 2.75 }, py: 1.5, borderTop: '1px solid', borderColor: 'divider' }}><Button onClick={() => isCreate ? navigate('/notices') : setEditing(false)}>취소</Button><Button data-testid="notice-save-button" type="submit" variant="contained" disabled={saving || !form.title.trim() || !form.content.trim()}>{saving ? '등록 중…' : isCreate ? '등록' : '저장'}</Button></Box>
            </Box>
          </Card>
        ) : notice ? (
