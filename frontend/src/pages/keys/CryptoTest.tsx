...
import { mockKeys } from '../../mocks/adminData'
...
function CryptoTest() {
  ...
  const initialKey = searchParams.get('key') ?? mockKeys.find((key) => key.status === 'ACTIVE' && key.algorithm === 'AES')?.keyUid ?? ''
  ...
  const selectedKey = useMemo(() => mockKeys.find((key) => key.keyUid === keyUid), [keyUid])
  ...
  return (
    <Box>
      <PageHeader title="암복호화 테스트" description="KMS 관리 키를 언래핑해 동작을 검증합니다. ACTIVE 상태의 AES 키만 실행할 수 있으며 모든 호출은 key_usage_log와 감사로그에 기록됩니다." />
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0, 1.4fr) minmax(300px, 0.6fr)' }, gap: 2.5 }}>
        <Card>
          ...
          <CardContent sx={{ p: { xs: 2.5, sm: 3.5 } }}>
            <FormControl fullWidth sx={{ mb: 2.5 }}><InputLabel>관리 키 선택</InputLabel><Select label="관리 키 선택" value={keyUid} onChange={(event) => setKeyUid(event.target.value)}>{mockKeys.map((key) => <MenuItem key={key.keyUid} value={key.keyUid} disabled={key.status !== 'ACTIVE' || key.algorithm !== 'AES'}>{key.keyName} · {key.algorithm}-{key.keySize} · {key.status}</MenuItem>)}</Select></FormControl>
            {selectedKey && !executable && <Alert severity="error" sx={{ mb: 2 }}>선택한 키는 {selectedKey.status}/{selectedKey.algorithm} 상태이므로 테스트할 수 없습니다.</Alert>}
