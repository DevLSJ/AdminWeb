  ...
  return (
    <Box>
      ...
      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', xl: 'minmax(0,1.55fr) minmax(360px,.7fr)' }, alignItems: 'start', gap: 2 }}>
        <Card className="section-card" sx={{ maxHeight: { xl: 'calc(100dvh - 190px)' }, display: { xl: 'flex' }, flexDirection: 'column', overflow: { xl: 'hidden' } }}>
          <Tabs value={mode} onChange={(_event, value: 'encrypt' | 'decrypt') => { setMode(value); setResult(''); setError('') }} sx={{ px: 2.5, flexShrink: 0, borderBottom: 1, borderColor: 'divider' }}><Tab icon={<LockRounded />} iconPosition="start" label="암호화" value="encrypt" /><Tab icon={<LockOpenRounded />} iconPosition="start" label="복호화" value="decrypt" /></Tabs>
          <CardContent sx={{ minHeight: 0, p: { xs: 2.5, sm: 3.5 }, overflowY: { xl: 'auto' }, overscrollBehavior: 'contain' }}>
