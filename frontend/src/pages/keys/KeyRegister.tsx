...
import { ArrowBackRounded, KeyRounded, LockRounded } from '@mui/icons-material'
import {
  Alert,
  Box,
  Button,
...
const initialForm = {
  keyName: '',
  algorithm: 'AES' as KeyAlgorithm,
  keySize: 256,
  purpose: 'ENCRYPT' as KeyPurpose,
  expireAt: '2027-08-19',
...
function KeyRegister() {
  ...
  return (
    <Box>
      <PageHeader
        title="키 등록"
        description="SecureRandom으로 관리 키를 생성하고 마스터키로 AES-256-GCM 래핑해 저장합니다."
        action={<Button startIcon={<ArrowBackRounded />} onClick={() => navigate('/keys')}>목록으로</Button>}
      />

      <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0, 1.5fr) minmax(300px, 0.7fr)' }, gap: 2.5 }}>
        <Card>
          <CardContent sx={{ p: { xs: 2.5, sm: 3.5 } }}>
            <Box component="form" onSubmit={handleSubmit}>
              <Typography variant="h6" sx={{ mb: 2.5 }}>키 메타정보</Typography>
              <TextField fullWidth required label="키 이름 (keyName)" placeholder="예: PAYMENT-AES-010" value={form.keyName} onChange={(event) => setForm((current) => ({ ...current, keyName: event.target.value }))} helperText="영문 대문자, 숫자, 하이픈을 사용한 식별 가능한 이름을 권장합니다." sx={{ mb: 2 }} />
              <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: 'repeat(2, 1fr)' }, gap: 2, mb: 2 }}>
                <FormControl fullWidth><InputLabel>알고리즘 (algorithm)</InputLabel><Select label="알고리즘 (algorithm)" value={form.algorithm} onChange={(event) => changeAlgorithm(event.target.value as KeyAlgorithm)}><MenuItem value="AES">AES</MenuItem><MenuItem value="HMAC">HMAC</MenuItem><MenuItem value="RSA">RSA</MenuItem></Select></FormControl>
                <FormControl fullWidth><InputLabel>키 길이 (keySize)</InputLabel><Select label="키 길이 (keySize)" value={form.keySize} onChange={(event) => setForm((current) => ({ ...current, keySize: Number(event.target.value) }))}>{keySizeOptions.map((size) => <MenuItem key={size} value={size}>{size} bit</MenuItem>)}</Select></FormControl>
        ...
        <Stack spacing={2}>
          <Card><CardContent sx={{ p: 3 }}><Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 2 }}><LockRounded color="primary" /><Typography variant="h6">저장 보안</Typography></Box><Stack spacing={1.2}><Chip label="키 평문 DB 저장 금지" color="error" variant="outlined" /><Chip label="마스터키 AES-256-GCM 래핑" color="primary" variant="outlined" /><Chip label="HMAC-SHA256 무결성 생성" color="success" variant="outlined" /></Stack></CardContent></Card>
