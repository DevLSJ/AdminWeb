...
import { canDecryptWithStatus, canEncryptWithStatus, getManualKeyStatusTransitions } from '../../utils/keyLifecycle'
...
function KeyDetail() {
  ...
  useEffect(() => {
    setRotationDraft(autoRotation ? String(autoRotation) : '')
  }, [autoRotation])
  ...
  const updateAutoRotation = async (value: string | number) => {
    if (!key) return
    const days: AutoRotationDays = value === 'NONE' ? null : Number(value)
    try {
  ...
  return (
    <Box>
      ...
      {tab === 0 ? (
        <Box key="overview" className="page-route-transition" sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: 'minmax(0, 1.35fr) minmax(320px, 0.65fr)' }, gap: 2.5 }}>
          <Stack spacing={2.5}>
            <Card sx={{ borderColor: key.integrityValid ? 'divider' : 'error.main', boxShadow: key.integrityValid ? undefined : '0 0 0 3px rgba(228, 81, 111, 0.10)' }}><CardContent sx={{ p: 2.25 }}><Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 0.75 }}><Typography variant="h6">키 메타정보</Typography><StatusBadge status={key.status} /></Box><Divider /><InfoRow label="키 UID" value={<Typography sx={{ fontFamily: 'monospace', fontSize: 12, wordBreak: 'break-all' }}>{key.keyUid}</Typography>} /><InfoRow label="키 이름" value={key.keyName} /><InfoRow label="알고리즘" value={`${key.algorithm}-${key.keySize} / ${key.mode}`} /><InfoRow label="용도" value={key.purpose} /><InfoRow label="현재 버전" value={`v${key.version}`} /><InfoRow label="자동 갱신" value={key.autoRotationDays ? `${key.autoRotationDays}일` : '미사용'} /><InfoRow label="만료일" value={key.expireAt} /><InfoRow label="무결성" value={<StatusBadge status={key.integrityValid ? 'VALID' : 'INVALID'} label={key.integrityValid ? 'HMAC 검증 정상' : '즉시 격리 필요'} icon={<ShieldRounded />} />} /></CardContent></Card>
            <Card><CardContent sx={{ p: 2.25 }}><Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 1, mb: 1.5 }}><Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}><HistoryRounded color="primary" /><Typography variant="h6">생명주기 타임라인</Typography></Box><StatusBadge label={`${histories.length}건`} tone="neutral" minWidth={0} /></Box><Box className="timeline-scroll" sx={{ maxHeight: 330, overflowY: 'auto', overscrollBehavior: 'contain', pr: 1 }}>{histories.map((history, index) => <Box key={history.id} sx={{ display: 'grid', gridTemplateColumns: '18px 1fr', gap: 1.25 }}><Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}><Box sx={{ width: 9, height: 9, borderRadius: '50%', bgcolor: 'primary.main', mt: 0.65 }} />{index < histories.length - 1 && <Box sx={{ width: 2, flex: 1, minHeight: 48, bgcolor: 'divider' }} />}</Box><Box sx={{ pb: 2 }}><Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 0.7, alignItems: 'center' }}><Typography sx={{ fontWeight: 750, fontSize: 13.25 }}>{history.fromStatus ? getStatusLabel(history.fromStatus) : '초기'} → {getStatusLabel(history.toStatus)}</Typography><StatusBadge label={`${history.operation} · v${history.keyVersion}`} tone="neutral" minWidth={0} /></Box><Typography sx={{ mt: 0.35, color: 'text.secondary', fontSize: 12.25 }}>{history.reason}</Typography><Typography sx={{ mt: 0.45, color: 'text.secondary', fontSize: 11.5 }}>{history.changedAt} · {history.changedBy}</Typography></Box></Box>)}</Box></CardContent></Card>
          </Stack>
          <Stack spacing={2.5}>
            ...
            <Card>
              <CardContent sx={{ p: 3 }}>
                ...
                <Stack spacing={1.2} sx={{ mt: 2 }}>
                  ...
                  <Button fullWidth variant="outlined" disabled={!key.integrityValid || (!canEncryptWithStatus(key.status) && !canDecryptWithStatus(key.status))} onClick={() => navigate(`/keys/test?key=${key.keyUid}`)}>암복호화 테스트</Button>
                  {isAdmin && (
        ...
        <Stack key="versions" className="page-route-transition" spacing={2.5}>
          <Card><CardContent sx={{ p: 2.5 }}><Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', justifyContent: 'space-between', gap: 2 }}><Box><Typography variant="h6">키 갱신 및 자동 순환</Typography><Typography sx={{ mt: 0.4, color: 'text.secondary', fontSize: 13 }}>신규 버전은 암호화에 사용하고 이전 버전은 복호화 전용으로 보존합니다.</Typography></Box>{isAdmin && <Button variant="contained" startIcon={<AutorenewRounded />} disabled={!key.integrityValid || !canEncryptWithStatus(key.status)} onClick={() => setRotateOpen(true)}>수동 키 갱신</Button>}</Box><Divider sx={{ my: 2 }} /><Box sx={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 1.5 }}><Typography sx={{ minWidth: 130, fontSize: 13.5, fontWeight: 700 }}>자동 갱신 주기</Typography><TextField size="small" type="number" value={rotationDraft} placeholder="미사용" disabled={!isAdmin} onChange={(event) => setRotationDraft(event.target.value)} slotProps={{ htmlInput: { min: 1, max: 3650 } }} sx={{ width: 150 }} /><Button size="small" variant="outlined" disabled={!isAdmin || (rotationDraft !== '' && (Number(rotationDraft) < 1 || Number(rotationDraft) > 3650))} onClick={() => void updateAutoRotation(rotationDraft || 'NONE')}>정책 적용</Button><Typography sx={{ color: 'text.secondary', fontSize: 12.5 }}>1~3650일 · 비우면 미사용</Typography></Box></CardContent></Card>
