      ...
      <Card className="list-results">
        <TableContainer sx={paginatedTableContainerSx(params.size, 390)}>
          <Table stickyHeader size="small" sx={{ minWidth: 920, tableLayout: 'fixed', '& th, & td': { verticalAlign: 'middle', ...paginatedTableCellSx(params.size) }, '& th': { py: 0.9 } }}>
            <TableHead><TableRow><TableCell align="center" sx={{ width: '6%' }}>#</TableCell><TableCell sx={{ width: '17%' }}>시각 (KST)</TableCell><TableCell sx={{ width: '11%' }}>행위자</TableCell><TableCell sx={{ width: '17%' }}>행위</TableCell><TableCell sx={{ width: '15%' }}>대상 유형</TableCell><TableCell sx={{ width: '24%' }}>설명</TableCell><TableCell align="center" sx={{ width: '10%' }}>행 HMAC</TableCell></TableRow></TableHead>
            <TableBody>
              ...
              {!loading && pageData.content.map((log, index) => {
                const invalid = !log.chainValid || invalidLogUids.has(log.logUid)
                return <TableRow key={log.logUid} hover tabIndex={0} className="interactive-row" onClick={() => { setDetail(log); setEntryVerification(null); void verifyEntry(log) }} onKeyDown={(event) => { if (event.key === 'Enter' || event.key === ' ') { setDetail(log); setEntryVerification(null); void verifyEntry(log) } }} sx={{ cursor: 'pointer', ...(invalid ? { bgcolor: 'rgba(228, 81, 111, 0.09)' } : {}) }}><TableCell align="center" sx={{ color: 'text.secondary', fontVariantNumeric: 'tabular-nums' }}>{log.displayNumber ?? pageData.totalElements - params.page * params.size - index}</TableCell><TableCell sx={{ whiteSpace: 'nowrap', color: invalid ? 'error.main' : 'text.primary', fontWeight: invalid ? 700 : 400 }}>{formatKst(log.createdAt)}</TableCell><TableCell><StatusBadge label={log.actor} tone="neutral" /></TableCell><TableCell><Typography noWrap sx={{ fontWeight: 700, color: invalid || log.action === 'USER_VIEW_PLAIN' ? 'error.main' : 'text.primary', fontSize: 13.5 }}>{auditActionLabels[log.action] ?? log.action}</Typography></TableCell><TableCell><Typography noWrap sx={{ fontSize: 13.5 }}>{log.targetType}</Typography></TableCell><TableCell><Typography noWrap sx={{ fontSize: 13.5 }}>{log.detail}</Typography></TableCell><TableCell align="center">{verifyingUid === log.logUid ? <CircularProgress size={18} /> : <StatusBadge dot status={invalid ? 'INVALID' : 'VALID'} />}</TableCell></TableRow>
              })}
            </TableBody>
          </Table>
        </TableContainer>
