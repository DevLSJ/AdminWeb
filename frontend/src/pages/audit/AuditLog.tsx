    ...
    <Box>
      ...
      <Drawer anchor="right" open={Boolean(detail)} onClose={() => { setDetail(null); setEntryVerification(null) }} slotProps={{ backdrop: { sx: { bgcolor: 'rgba(20,29,48,.34)', backdropFilter: 'blur(5px)' } }, paper: { sx: { width: { xs: '100%', sm: 720, xl: 880 }, maxWidth: '100%', borderRadius: { sm: '18px 0 0 18px' }, boxShadow: '-24px 0 64px rgba(25,42,78,.2)' } } }}>
        <Box sx={{ height: '100%', overflowY: 'auto' }}>
          ...
          {detail && (
            <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', md: 'repeat(2, minmax(0,1fr))' }, alignItems: 'start', gap: 2, p: 3 }}>
              <Box sx={{ minWidth: 0, p: 2.25, border: '1px solid', borderColor: 'divider', bgcolor: 'background.paper' }}>
                ...
                <InfoRow label="로그 UID" value={detail.logUid} /><InfoRow label="행위자" value={detail.actor} /><InfoRow label="행위" value={auditActionLabels[detail.action] ?? detail.action} /><InfoRow label="대상 유형" value={detail.targetType} /><InfoRow label="대상 ID" value={detail.targetId} /><InfoRow label="기록 시각" value={`${formatKst(detail.createdAt)} KST`} />
