...
function Dashboard() {
  ...
  return <Box sx={{ height: { xs: 'calc(100dvh - 104px)', sm: 'calc(100dvh - 112px)' }, display: 'flex', flexDirection: 'column', gap: 1.5, overflow: 'hidden', minHeight: 0 }}>
    <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 1, flexShrink: 0, flexWrap: 'wrap' }}><Box><Typography variant="h5">대시보드</Typography><Typography sx={{ mt: .5, color: 'text.secondary', fontSize: 12.5 }}>D&apos;Guard KMS 운영 현황과 조치가 필요한 항목을 한 화면에서 확인합니다.</Typography></Box><Stack direction="row" spacing={1} sx={{ flexWrap: "wrap", gap: 1 }}>{isAdminRole(user?.role) && <Button variant="outlined" onClick={() => navigate("/keys")}>키 등록</Button>}<Button variant="outlined" onClick={() => navigate("/notices")}>게시글 등록</Button>{isAdminRole(user?.role) && <Button variant="contained" onClick={() => navigate("/users?create=1")}>사용자 등록</Button>}</Stack></Box>
    ...
    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: 'minmax(0,1.6fr) minmax(0,1fr)' }, gap: 1.5, flex: '1.1 1 0', minHeight: 0 }}>
