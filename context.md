...
`GET /api/audit-logs/verify`는 `from`, `to` 일시 범위를 받아 ID 오름차순으로 선택 로그를 읽고 다음 항목을 확인한다. 범위를 생략하면 기존처럼 전체 체인을 검증한다.
...
기간 검증에서는 첫 선택 행과 범위 밖 직전 행의 연결, 마지막 선택 행과 범위 밖 다음 행의 연결까지 확인한다. 선택한 마지막 행이 전체 마지막 행일 때만 `audit_chain_head.current_hash`를 확인하므로 범위를 잘라 검사해도 경계의 삭제·변조를 놓치지 않는다.
