...
- 키 카테고리는 대칭 암호화(AES), 비대칭 암호화(RSA-OAEP), 메시지 인증(HMAC-SHA256)으로 구분한다. 화면 상태는 생성됨, 활성화, 비활성, 폐기로 단순화하고 재활성·만료·운영 중지·배포 상태는 D’Guard 정책 확장으로 정규화한다.
...
- 웹 목록과 상세 화면의 상태를 `생성됨(CREATED)`, `활성화(ACTIVE)`, `비활성(DEACTIVATED)`, `폐기(DESTROYED)` 4개로 정규화했다. 기존 `REACTIVATED`·`DISTRIBUTED`는 활성화, `EXPIRED`·`INACTIVE`는 비활성으로 표시해 기존 DB도 누락 없이 조회한다.
- 키 상세의 생명주기 작업 영역은 현재 상태만 표시한다. 생명주기 타임라인은 내부 스크롤을 유지하면서 작업 코드(`STATUS_CHANGE` 등)를 숨기고 버전 `v1`, `v2`만 표시한다.
- 활성 키만 암호화·복호화할 수 있다. 비활성 키는 두 작업을 모두 차단하되 자동·수동 회전은 계속 허용하고 새 버전 이력을 누적한다.
...
- 키 카테고리 필터는 대칭키와 공개키로 단순화하고, 대시보드 상태 분포도 동일한 4개 상태로 합산한다. 기존 레거시 키와 HMAC 데이터는 전체 조회에서 보존한다.
...
## 18. 사용자 등록 암호 정책 강화 (2026-09-02)

- 등록 비밀번호는 사용자별 16바이트 난수 Salt와 PBKDF2-HMAC-SHA256 256비트 해시만 저장하며, 반복 횟수는 애플리케이션 기동 시 210,000회 이상을 강제한다. 기준보다 낮은 환경설정은 서버 기동 단계에서 거부한다.
- 이름·연락처·이메일은 서버 마스터키로 AES-256-GCM 암호화한다. 각 필드마다 12바이트 난수 IV를 새로 생성하므로 같은 연락처를 다시 저장해도 암호문과 IV는 바뀐다.
- 연락처 정확검색과 중복 검사는 숫자만 정규화한 값의 도메인 분리 HMAC으로 처리한다. 재암호화 시 검색 HMAC은 유지되지만, API와 일반 화면에는 마스킹 값만 반환한다.
- `V13__enforce_app_user_crypto_policy.sql`은 PostgreSQL에서도 PBKDF2 알고리즘·최소 반복 횟수, GCM IV·암호문 길이를 체크 제약으로 강제한다.
- 운영 배포 전 기존 사용자 2건을 읽기 전용 점검한 결과 약한 비밀번호 정책, 잘못된 IV 길이, 비정상 암호문이 모두 0건이었다. 동일 마이그레이션 문장을 트랜잭션에서 적용 후 롤백하는 사전 검증도 성공했다.

## 19. 통합 사용자 목록·권한 관리 (2026-09-04)

- 사용자 관리 목록은 프론트에서 `admin_user`와 `app_user`를 별도 요청해 임시 병합하지 않는다. `/api/users/managed`가 두 테이블을 한 번에 결합해 정렬·검색·상태 필터·서버 페이징 결과를 반환한다.
- 관리 계정과 암호화 사용자는 같은 목록 행 및 우측 상세 패널 구조를 사용한다. 이름, 권한, 상태, 등록일, 무결성 상태와 수정·비밀번호 재설정·상태 변경 작업 위치를 통일한다.
- 암호화 사용자 등록·수정에는 `ADMIN` 또는 `CLIENT` 권한을 포함한다. 기존 `app_user` 행은 V14 마이그레이션에서 최소 권한인 `CLIENT`로 설정한다.
...
## 21. 사용자 편집·목록 밀도 통일 (2026-09-04)

- 기존 관리 계정 3건도 서비스 사용자와 같은 상세 수정 화면에서 연락처·이메일을 등록하거나 교체할 수 있다. `admin_user`에는 평문 대신 AES-256-GCM 암호문, 필드별 12바이트 IV, 마스킹 값과 암호화 버전을 저장한다.
- 관리 계정 행 무결성 형식을 V2로 확장해 암호화 연락처·이메일도 HMAC 서명 대상에 포함한다. 기존 V1 행은 서명이 정상이고 보호 연락처가 없는 경우에만 V2로 재서명한다.
- 공지 등록·수정의 본문 라벨은 항상 축소 상태와 배경 여백을 유지해 포커스 시 외곽선 및 입력 본문과 겹치지 않는다.
- 최근 활동, 키 목록, 감사 로그, 공지사항 목록의 페이지 크기 선택을 `10개씩`, `20개씩`, `30개씩`으로 통일한다. 행 간격과 최대 높이는 선택 크기에 따라 부드럽게 바뀌고, 화면에 무리하게 압축되는 구간은 내부 세로 스크롤을 사용한다.

## 22. 목록 열 기준선·타임라인 정렬 보강 (2026-09-04)

- 최근 활동의 결과 열과 키 목록의 관리 열은 헤더·본문에 같은 고정 폭과 중앙 정렬을 사용해 상태 배지 및 관리 버튼이 제목 바로 아래에 놓인다.
- 암복호화 테스트의 생명주기 타임라인은 우측 68px 열을 공통으로 사용해 헤더의 이력 건수와 각 행의 키 버전 배지를 같은 세로축에 배치한다.
- 사용자 관리의 최근 접속일과 공지사항의 조회수는 헤더와 값을 함께 중앙 정렬해 우측 가장자리 쏠림을 제거한다.
- 공지사항 목록은 전체 폭 고정 레이아웃을 유지하면서 최소 폭, 열 너비, 헤더·본문 글자 크기와 셀 여백을 다른 관리 목록 수준으로 확대한다.

## 23. 대시보드 그래프·통계 카드 배치 보강 (2026-09-04)

- 대시보드와 키 통계의 사용 추이 그래프는 일·월 데이터 지점 수와 관계없이 같은 플롯 캔버스 크기를 유지한다. 기간을 바꿔도 차트 전체가 줄어들지 않고 데이터 지점 간격만 자동 조정된다.
- 키 통계에서 `작업 성공률` 카드를 제거하고, 전체 관리 키·기간 내 키 사용·만료 임박·무결성 위반 4개 카드가 대형 화면의 한 줄을 균등하게 채운다.

## 24. 심화 코드·정책 관리와 키 목록 표시 (2026-09-09)

- `/settings`에 기존 디자인을 재사용한 코드·정책 관리 화면을 추가했다. S.ADMIN 변경·ADMIN 조회, 설정 GET API는 인증 사용자에게 제공한다.
- V19의 `common_code`·`key_policy`에 표시명·정렬·신규 선택 허용 여부와 기본 유효기간 365일·만료 알림 30일을 저장한다. 기존 키·만료일·암호화 설정은 소급 수정하지 않는다.
- 신규 생성의 알고리즘/용도 조합과 설정 활성 여부를 서버에서 검증한다. 기존 키의 암복호화·회전·상태 전이는 보존하며, 임의 알고리즘 추가와 상태 전이 수정은 허용하지 않는다.
- 정책 변경은 사유·변경 전후 값을 감사 체인에 기록하고 version으로 동시 변경을 검출한다. 자세한 계약·적용 범위·시연은 `docs/KEY_SETTINGS.md` 참고.
- 키 목록의 정책상 만료 임박 ACTIVE 행을 연한 붉은색으로 표시한다. 무결성 오류 강조는 우선 유지하고, 폐기는 회색 휴지통 아이콘을 사용한다.
- 새 키 등록 달력은 선택된 만료일과 별개로 열 때마다 현재 시스템 월을 표시한다. 다음 달로 날짜가 바뀐 후 다시 열 때도 현재 월을 따른다. 기존 날짜 바로가기와 등록 기본 만료일은 유지한다.
- 검증: 전체 백엔드 61/61, 임시 PostgreSQL + V1~V19 정책·4주차 29/29, 프런트 빌드·lint, 실제 Chrome 5개 시나리오 통과. 운영 배포는 실행하지 않았다.

## 25. 운영 정책 UI·생성 순서 번호·상세 만료 표시 (2026-09-09)

- 사이드바와 화면 제목을 `운영 정책 관리`로 통일하고, `키 운영 정책` 카드 제목을 세로 중앙에 정렬했다. 코드 편집 문구는 `신규 등록 허용`·`변경 사유`로 변경했다.
- 키·사용자·감사·게시글 목록의 `#`은 최신 생성 항목이 가장 큰 번호다. 키·게시글·감사는 DB 생성 순번, 사용자는 두 계정 테이블 전체 생성 시각순 순위를 사용한다. 검색·정렬·페이징으로 번호를 다시 부여하지 않으며 게시글 공지 우선 정렬은 유지한다. API 식별자는 기존 UUID를 유지한다.
- 만료 임박 ACTIVE 키 상세의 이름 옆에 시계 아이콘과 만료일 툴팁을 추가했다. 정책 알림일과 KST 기준을 재사용하며 키 목록의 만료 강조·폐기 아이콘은 변경하지 않았다.
- 검증: 전체 백엔드 65/65, PostgreSQL 목록 번호 통합 4/4, 프런트 빌드·lint, 실제 Chrome 정책·달력·상세 아이콘 6개 시나리오 통과.
- DB 스키마·Docker·CI 설정 변경은 없다. 프런트와 백엔드 응답 변경이므로 두 서비스가 배포 대상이다.
